...
export default function EditEvent() {
    ...
    async function load() {
      ...
      if (event) {
        setForm({
          ...
          price_nonmember: event.price_nonmember ?? "",
          seats: event.seats ?? "",
      ...
      if (opts) setOptions(opts.map((o) => ({ label: o.label, price: String(o.price), onsite_price: o.onsite_price != null ? String(o.onsite_price) : "", payment_link: o.payment_link || "" })));
  ...
  if (loadingEvent || !form) return null;

  if (success) {
    return (
      <div style={{ padding: "60px 20px", textAlign: "center" }}>
        <Check size={40} color={colors.olive} style={{ marginBottom: 12 }} />
        <p style={{ fontSize: 15, fontWeight: 700 }}>Événement mis à jour !</p>
        <p style={{ fontSize: 13, color: colors.muted }}>Retour à l'administration…</p>
      </div>
    );
  }
  ...
  return (
    <div style={{ padding: "0 20px 60px" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "18px 0 16px" }}>
        ...
        <h1 style={{ fontFamily: fonts.display, fontSize: 19, margin: 0 }}>Modifier l'événement</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        <div>
          ...
          <select style={inputStyle} value={form.venueId} onChange={(e) => update("venueId", e.target.value)}>
        ...
        <div style={{ border: `1px solid ${colors.border}`, borderRadius: 14, padding: 14, background: colors.surface }}>
          ...
          />
          <p style={{ fontSize: 11, color: colors.muted, marginTop: 6, lineHeight: 1.4 }}>
            Si renseigné, remplace le système de paiement intégré (les places ne sont plus comptées). Les options restent affichées sur la page de l'événement, avec leur propre lien de paiement si indiqué.
          </p>
        </div>
